...
- 评论区支持普通用户发表评论；管理员使用交易管理密码进入管理员模式后，可以发表带管理员标识的评论、回复以及删除评论。删除主评论时会同时删除其回复。
...
| `COMMENTS_ENABLED` | 是否允许访客发表评论；关闭后管理员仍可发公告、回复和删除 | `true` |
...
- 修复 JS 语法错误：Python 三重引号里的 `\n` 被渲染成 JS 单引号字符串内的字面换行，导致页面所有 JS 无法执行、登录完全没反应。
- 登录按钮增加 onclick 保底机制，并新增 JS 加载状态指示。
- 修复防守震荡策略忽略 `auto_sell` 标志：手动设置为"取消自动卖"的批次，价格触及防守震荡目标时仍被自动卖出。

### v1.0.17 - 2026-06-07

- 新增最终卖出保险：自动 agent 在提交任何卖单前，用账本真实成本 + 双边手续费复核，卖出价低于保本价时强制 HOLD，避免旧批次因目标价错误而亏损卖出。
- 自动 agent 卖出路径中的币安 `-2010` 余额不足错误改为中文提示，并显示当前可用余额。
- 修复已平批次顺序：改为按平仓时间降序排列，最近平仓的批次排在最前面。

### v1.0.16 - 2026-06-07

- 修复多批次限价卖出时币安返回 `-2010 insufficient balance` 的问题：挂单会占用并锁定 BTC，后续批次现在按账户实时可用余额计算。
- 买入手续费若由 BTC 扣除，账本改为记录扣除手续费后的实际到账数量，避免长期累计后账本数量高于真实余额。
- 单批次可用 BTC 不足时允许按剩余可用量挂部分限价单，并在挂单记录中保存原请求数量与实际挂单数量。
- 可用量不足最小下单额时返回明确中文提示，展示可用和被其他挂单锁定的 BTC，不再直接显示币安 `-2010`。
- 一键限价卖改为逐批完成并汇总成功、跳过和失败数量，不会因为其中一笔失败而立即终止全部操作。
- 新增 BTC 手续费净数量和锁定余额部分挂单测试。

### v1.0.15 - 2026-06-07

- 修复波段批次预计卖价可能低于真实成本价的问题；自动卖价现在至少覆盖波段最小利润空间和双边手续费。
- 已有波段批次若保存了过低目标，会自动启用“防亏保护”并在看板显示修正后的安全卖价。
- 限价挂单表新增关联批次、成本价和预计净收益，能够直接判断每张卖单对应哪个批次以及按挂单价成交后的预估结果。
- 限价买单显示“成交后新建批次”和预计买入成本；脚本外部持仓卖单明确标识为“外部持仓”。
- 新增波段旧目标防亏与限价挂单成本收益测试；全量 64 项测试通过。

### v1.0.14 - 2026-06-07

- 修复防守震荡批次可能亏损卖出的问题：目标价不再沿用下单前行情价，而是按真实买入成交均价重新计算。
- 为已有防守震荡批次增加卖出安全线，预计卖价至少覆盖双边手续费和策略目标利润；旧错误目标不会再触发自动卖出。
- 看板会将被修正的旧批次标记为“防亏保护”，显示实际采用的安全卖价。
- 面板安全设置新增“允许访客评论”开关；关闭后普通用户不能发表评论，但管理员仍可发布公告、回复和删除。
- 删除 Web Manifest 引用和 `/manifest.webmanifest` 接口，保留普通网页图标与 iOS Touch Icon。
- 新增真实成交价重算、旧批次防亏卖出和评论关闭权限测试；全量 62 项测试通过。

### v1.0.13 - 2026-06-07

- 评论区新增“管理员模式”，使用交易管理密码验证管理员身份。
- 管理员可以发表带“管理员”标识的主评论，也可以回复普通用户评论。
- 管理员可以删除单条回复或整条主评论；删除主评论时会同步清理该评论下的全部回复。
- 普通用户只能发表评论，不能使用删除功能；管理员发言、回复和删除均由服务端再次校验管理密码。
- 管理员权限仅保留在当前页面会话中，不写入长期存储；关闭页面后需要重新验证。
- 删除评论前增加页面内确认窗口，降低误删风险。
- 新增评论删除与级联清理测试；全量 59 项测试通过。

### v1.0.12 - 2026-06-06

- 登录新增“信任此设备”模式，验证成功后长期免密，适合个人手机和电脑。
- 原“记住我”明确为临时设备模式，只保持 24 小时；两种模式互斥。
- 主题面板新增“取消信任并退出”，可主动清除长期凭据并返回登录页。
- GitHub 仓库随品牌更名为 `yishuizhe/spotflow`。

### v1.0.11 - 2026-06-06

- 网页标题改为 `SpotFlow · 现货量化助手`，iPhone 添加到主屏幕后使用简洁名称 `SpotFlow`。
- 页面顶部品牌同步改为 `SpotFlow`，副标题为“现货量化交易看板”。
- 新增 iOS 专用 180×180 Apple Touch Icon，以及 192×192、512×512 Web App PNG 图标。
- 新增 Web App Manifest 和 iOS 独立应用模式元信息，修复 Safari 使用灰色字母 `B` 作为桌面图标的问题。

### v1.0.10 - 2026-06-06

- 缩小右侧浮动工具台及其图标尺寸，并将工具台从页面中部移动到右下角。
- 主题抽屉改为从右下角向左展开，手机端同步缩小，减少对页面内容的遮挡。
...
- 修正右侧主题按钮语义：删除容易被理解为简繁切换的“繁”字，改为明确的调色盘图标。
- 删除主题抽屉中重复的“系统设置”按钮，系统设置只保留右侧齿轮入口。

### v1.0.7 - 2026-06-06

- 看板默认改为更紧凑的内容宽度，为左侧助手和右侧快捷工具栏保留空间；用户也可以通过宽度按钮切换宽屏布局。
- 右侧设置入口重做为竖向快捷工具栏，包含主题、明暗模式、页面宽度、系统设置和回到顶部。
- 修正主题快捷按钮的错误“繁”字图标，改为明确的调色盘图标，并移除主题抽屉中重复的系统设置入口。
- 所有人工交易、批次操作、校准和设置密码输入均改为页面内窗口，不再调用浏览器原生弹窗。
- 修复设置保存后可能一直停留在“保存中”的问题，无论保存或重新读取是否成功都会恢复按钮状态。
- “记住我”改为无等待进入页面，首次真实接口请求会自然校验缓存；失效时自动清理并显示登录窗口。
- 扩充看板助手说明范围，覆盖主要按钮、表头、账户字段、策略摘要和设置字段。
- 新增 SQLite 评论区、弹幕显示、弹幕开关与速度设置；鼠标悬停弹幕会暂停。作者回复需要交易管理密码并显示作者标识。
- 角色图片增加缓存版本和失败自动重试，连续失败时显示文字占位，避免永久出现破图问号。
- 修正 SQLite 存储连接未及时关闭的问题，降低长期运行时积累数据库连接和文件句柄的风险。
- 行情上游临时断开时，状态接口会返回规范 JSON 错误，不再因连接异常留下半截响应。

### v1.0.6 - 2026-06-05

- 合并 PR #3：在顶部新增紧凑的策略摘要，展示止盈、买入间距、仓位分档、防守模式与浮亏保护。
- 审核并关闭 PR #2：原实现会让所有弹窗操作栏变成粘性定位；本版本改为仅固定设置弹窗的保存操作区。
- 行情图支持使用滚轮在分时、5 分、15 分、1 小时、4 小时、1 日和 1 周之间连续切换，顶部标签会同步变化。
- 压缩顶部留白并重排核心资产、盈亏和交易控制区域，看板首屏更紧凑。
- 面板中的价格、金额、盈亏和回测结果统一显示两位小数，BTC 数量保留必要精度。
- 未平批次新增分页，桌面端四个批次操作按钮保持同一排；看板助手进一步向左调整。
- 修正部署清单，服务器升级时会同步 `CHANGELOG.md`，避免程序版本与服务器内更新日志不一致。

### v1.0.5 - 2026-06-05

- 合并并增强 PR #1（感谢 R0A1NG）：登录页新增可选的“记住我”，验证成功后可在当前浏览器保持登录 24 小时。
- 缓存过期、格式损坏或页面密码修改后会自动清除旧登录信息，不会反复使用失效密码。
- 长期缓存只会在服务器验证登录成功后写入，并明确提示仅建议在私人设备使用。

### v1.0.4 - 2026-06-05

- 强化系统设置保存反馈：保存期间按钮显示进行中状态，成功/失败会在页面顶部显示醒目提示条。
- 设置窗口内新增持久的成功/失败状态块，重新读取配置时不再覆盖刚刚的保存结果。
- 顶部提示默认停留 8 秒并支持手动关闭，避免用户错过保存结果。

### v1.0.3 - 2026-06-05

- 撤销看板助手动态眼睛效果，保留原始静态角色图，避免眼睛覆盖层影响观感。

### v1.0.2 - 2026-06-05

- 重做看板助手眼睛动画，撤掉夸张的大眼球覆盖，保留原角色眼睛，只叠加小尺寸同色虹膜微动层。
- 眼神跟随幅度改为轻微移动，避免斗鸡眼、惊恐眼和贴图穿帮。

### v1.0.1 - 2026-06-05

- 修复看板助手眼睛覆盖成黑点的问题，改为完整动态眼睛层，虹膜会跟随鼠标移动。
- 看板助手改为紧贴页面底部，左侧留出适中距离；对话气泡会自动收起，不再长期遮挡页面。
- 未平批次统一提供市价卖出、开启/取消自动卖、限价卖出、外部已卖四个操作。
- 未平批次顶部新增一键开启自动卖、一键关闭自动卖、一键市价卖、一键限价卖。
- 放开手动卖出和自动卖开关限制，所有账本未平批次都可以在面板上人工接管。
- 按中文市场习惯调整颜色：上涨/盈利为红色，下跌/亏损为绿色；交易开关仍保持开为绿色、停为红色。

### v1.0.0 - 2026-06-05

- 将项目版本升级为 `v1.0.0`，标记为完整实盘看板与策略工具的首个稳定学习版。
- 移除顶部设置按钮，只保留右下角博客式浮动设置入口，减少页面冲突和遮挡。
- 将主题面板改成小圆形入口加抽屉式展开，并新增森林、海洋、落日、极简等主题。
- 替换左下角助手为原创生成图像，并优化桌面和手机端尺寸。
- 增强看板助手交互：鼠标移动时眼睛跟随，悬停按钮、标题和指标时会解释对应含义。
- 将行情图 Y 轴放回左侧，并把近期买入/卖出点标成类似交易所的 `B` / `S` 圆点。
- 缩减最近实盘订单每页条数，让账户与策略、最近订单两侧观感更一致。

### v0.3.1 - 2026-06-05

- 未平/已平批次展示改为“策略档位 + 批次短编号”，避免同名档位看起来像重复记录。
- 最近实盘订单每页调整为 `13` 条，方向和批次名称改为中文展示。
- 行情走势升级为 K 线蜡烛图，补充开高低收数据、更多周期按钮，并在图上标注近期买入/卖出点。
- 账户与策略区域重写为简洁中文摘要，隐藏用户不需要的英文 reason 和内部字段。
- 设置弹窗关闭按钮固定在顶部，设置分类之间增加间距。
- 新增主题设置面板，支持白天、交易所深色、星界竞技、动画校园、樱音舞台等原创主题。
- 新增左下角原创看板助手，可隐藏；眼睛跟随鼠标，悬停按钮时会解释功能，并会提示近期 BTC 状态。
